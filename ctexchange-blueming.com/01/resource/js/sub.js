...
function  blockedNotNumber (event) {
    var blockedKeyList = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'];
    if(blockedKeyList.includes(event.key)){
...
function syncPhone2(event) {
    document.querySelectorAll(".phone_2").forEach(element => {
        element.value = event.target.value;
    });
}
function syncPhone3(event) {
    document.querySelectorAll(".phone_3").forEach(element => {
        element.value = event.target.value;
    });
}
